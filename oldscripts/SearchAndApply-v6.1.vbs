...
'v6 - Cleaned up Vars in script and also used image*.swm for split .WIM files. -2012Nov30
'-v6.1 - Removed -align argument on diskpart script when applying .WIM files. As it presented errors on CF53-2 -2013Jan11
...
aFileExt(2) = "swm"
'==========MAIN ROUTINE===================================
If fCheckDiskis100() = "False" Then 'Verify local disk is 100GB in size
	WScript.Echo Time & "  ::  Local Disk is less than 100GB, which means WinPE does not see the local Hard Drive.  Please use diskpart to verify this claim."
	WScript.Quit
End If

'Begin for Image files
Dim drv, drvcoll, drvlist, volSet
Set drvcoll = oFSO.Drives

s2SWMPath = ""
sCDDrvLetter = fGetCDRDrvLetter()
If oFSO.FileExists(sCDDrvLetter & "\BOOTMGR") Then
	Set f = oFSO.GetFolder(sCDDrvLetter & "\")
			Set fc = f.Files
			For Each f1 in fc
				'WScript.Echo f1.Name
				fileName = UCase(f1.Name)
				If RIGHT(fileName,3) = "WIM" Or RIGHT(fileName,3) = "GHO" then 
					If ImagePaths = "" Then
						ImagePaths = f1.path
					Else
						ImagePaths = ImagePaths & "," & f1.path
					End If
				ElseIf RIGHT(fileName,3) = "SWM" Then
					If RIGHT(fileName,5) = "2.SWM" Then
						s2SWMPath = f1.path
					Else
						ImagePaths = f1.path
					End If
...
For Each drv In drvcoll
	If drv.IsReady Then
		If drv.DriveType = 1 Then 'Drive is Removable
			Set f = oFSO.GetFolder(drv.DriveLetter & ":\")
			Set fc = f.Files
			For Each f1 in fc
				fileName = UCase(f1.Name)
				If RIGHT(fileName,3) = "WIM" Or RIGHT(fileName,3) = "GHO" then 
...
Function ApplyImagewImageX(imagePath)
	...
	'objansFile.WriteLine "cre par pri align=16065" 'Encountered issue with alignment on CF53-2
	objansFile.WriteLine "cre par pri"
	...
	If Not oFSO.FileExists("C:\boot.ini") Then
		...
		If oFSO.FolderExists("C:\Windows") Then
			sCmdPath = "bcdboot c:\windows"
			sIntReturn = oShell.Run(sCmdPath, 0, True)
		ElseIf oFSO.FolderExists("D:\Windows") Then
			sCmdPath = "bcdboot d:\windows"
			sIntReturn = oShell.Run(sCmdPath, 0, True)
		End If
